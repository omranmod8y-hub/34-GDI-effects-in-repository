...
def text_payload(stop, msg="Memoxide"):
    while not stop.is_set():
        ...
        win32gui.TextOut(hdc, random.randint(0, sw), random.randint(0, sh), msg)
        win32gui.ReleaseDC(0, hdc)
        time.sleep(0.05)
...
def cursor_text(stop):
    while not stop.is_set():
        ...
        win32gui.SetTextColor(hdc, win32api.RGB(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)))
        win32gui.TextOut(hdc, x, y, "Hello!")
        win32gui.ReleaseDC(0, hdc)
        time.sleep(0.01)
...
def sines(stop):
    ...
    while not stop.is_set():
        hdc = win32gui.GetDC(0)
        for i in range(0, sw + sh, 8): # Step 8 for performance
...
def main():
    if win32gui.MessageBox(0, "Run GDI Payload Test?", "Python Memoxide", win32con.MB_YESNO) != win32con.IDYES: return
    
    time.sleep(2)
    
    # 1. Tunnel
    run_phase([ran_tunnel], s1, 15)
    # 2. Colors
    run_phase([cube_color], s2, 15)
    # 3. Light + Move
    run_phase([light_effect, move_invert], s4, 15)
    # 4. Text + Icons
    run_phase([text_payload, icon_spam, cursor_text], s7, 15)
    # 5. Sines (Final)
    run_phase([sines], s11, 20)
